...
Public Module GanttDataProvider
	Private Const TasksSessionKey As String = "Tasks", DependenciesSessionKey As String = "Dependencies", ResourcesSessionKey As String = "Resources", ResourceAssignmentsSessionKey As String = "ResourceAssignments"
	...
	Public ReadOnly Property Tasks() As List(Of Task)
		Get
			If Session(TasksSessionKey) Is Nothing Then
				Session(TasksSessionKey) = CreateTasks()
			End If
			Return DirectCast(Session(TasksSessionKey), List(Of Task))
		End Get
	End Property
	Public ReadOnly Property Dependencies() As List(Of Dependency)
		Get
			If Session(DependenciesSessionKey) Is Nothing Then
				Session(DependenciesSessionKey) = CreateDependencies()
			End If
			Return DirectCast(Session(DependenciesSessionKey), List(Of Dependency))
		End Get
	End Property
	Public ReadOnly Property Resources() As List(Of Resource)
		Get
			If Session(ResourcesSessionKey) Is Nothing Then
				Session(ResourcesSessionKey) = CreateResources()
			End If
			Return DirectCast(Session(ResourcesSessionKey), List(Of Resource))
		End Get
	End Property
	Public ReadOnly Property ResourceAssignments() As List(Of ResourceAssignment)
		Get
			If Session(ResourceAssignmentsSessionKey) Is Nothing Then
				Session(ResourceAssignmentsSessionKey) = CreateResourceAssignments()
			End If
			Return DirectCast(Session(ResourceAssignmentsSessionKey), List(Of ResourceAssignment))
		End Get
	End Property
	...
	Private Function CreateDependencies() As List(Of Dependency)
		...
		For i As Integer = 0 To Tasks.Count - 1
			...
			If Not String.IsNullOrEmpty(task.ParentID) Then
				result.Add(New Dependency() With {
					.ID = CreateUniqueId(),
					.Type = 0,
					.ParentID = Tasks(i - 1).ID,
					.DependentID = task.ID
				})
			End If
		Next i
		...
		For Each task As Task In Tasks
			If Not String.IsNullOrEmpty(task.Employees) Then
				...
				For i As Integer = 0 To empIDs.Length - 1
					result.Add(New ResourceAssignment() With {
						.ID = CreateUniqueId(),
						.TaskID = task.ID,
						.ResourceID = empIDs(i)
					})
	...
	Public Sub DeleteTask(ByVal task As Task)
		...
		If taskToDelete IsNot Nothing Then
	...
	Public Sub DeleteDependency(ByVal dependency As Dependency)
		...
		If dependencyToDelete IsNot Nothing Then
	...
	Public Sub DeleteResource(ByVal resource As Resource)
		...
		If resourceToDelete IsNot Nothing Then
